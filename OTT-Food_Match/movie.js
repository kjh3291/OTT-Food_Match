...
const ottNameMapEs = { netflix: "Netflix", disney: "Disney+", tving: "TVING", wavve: "wavve" };
...
const mealMapEs = { "혼밥": "Comiendo Solo", "야식": "Snack Nocturno", "친구와 함께": "Con Amigos", "연인과 함께": "Con Pareja", "간단한 식사": "Comida Ligera", "든든한 식사": "Comida Pesada" };
...
function applyMovieLanguage() {
  ...
  } else if (lang === "es") { // 스페인어 로직 추가
    ottDisplay = ottNameMapEs[ottKey] || "OTT";
    mealDisplay = mealMapEs[selectedMeal] || selectedMeal;
  }
  ...
  if (moviePageTitle) {
    if (lang === "ko") // ...
    else if (lang === "es") moviePageTitle.textContent = `Películas en ${ottDisplay}`; // 추가
  }
  // 영화 페이지 안내 문구
  if (moviePageInfo) {
    if (lang === "ko") // ...
    else if (lang === "es") moviePageInfo.textContent = `Situación: ${mealDisplay} / Seleccione un botón de género arriba para filtrar películas.`; // 추가
  }
  // 장르 타이틀
  if (selectedGenreTitle) {
    if (lang === "ko") // ...
    else if (lang === "es") selectedGenreTitle.textContent = `Películas de ${typeof t === 'function' ? t("tab_" + genreKey) : genreKey}`; // 추가
  }
}
...
async function fetchMoviesFromTMDB(genre) {
  ...
  const lang = localStorage.getItem("lang") || "ko";
    ...
    zh: "zh-CN",
    ja: "ja-JP",
    es: "es-ES" // 추가 (스페인어 영화 데이터를 가져오기 위함)
  };
