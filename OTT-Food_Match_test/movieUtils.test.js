...
describe('sortMovies', () => {
  ...
  test('releaseDate가 없는 영화도 latest 정렬에서 오류 없이 동작한다', () => {
    // || "0000-00-00" 분기 커버
    const withMissing = [
      { title: '있음', popularity: 1, rating: 5, releaseDate: '2022-01-01' },
      { title: '없음', popularity: 2, rating: 5, releaseDate: null },
    ];
    const result = sortMovies(withMissing, 'latest');
    expect(result[0].title).toBe('있음');
    expect(result[1].title).toBe('없음');
  });
