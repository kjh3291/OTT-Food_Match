...
describe('makeRecommendReason', () => {
  test('액션 장르 추천 사유에 "액션" 키워드가 포함된다', () => {
    expect(makeRecommendReason('액션', '혼밥', '햄버거')).toContain('액션');
  });
  test('코미디 장르 추천 사유에 "코미디" 키워드가 포함된다', () => {
    expect(makeRecommendReason('코미디', '야식', '김치찌개')).toContain('코미디');
  });
  test('드라마 장르 추천 사유에 "드라마" 키워드가 포함된다', () => {
    expect(makeRecommendReason('드라마', '간단한 식사', '케이크')).toContain('드라마');
  });
  test('로맨스 장르 추천 사유에 "로맨스" 키워드가 포함된다', () => {
    expect(makeRecommendReason('로맨스', '연인과 함께', '마카롱')).toContain('로맨스');
  });
  test('스릴러 장르 추천 사유에 "스릴러" 키워드가 포함된다', () => {
    expect(makeRecommendReason('스릴러', '친구와 함께', '피자')).toContain('스릴러');
  });
  test('애니메이션 장르 추천 사유에 "애니메이션" 키워드가 포함된다', () => {
    expect(makeRecommendReason('애니메이션', '든든한 식사', '치킨')).toContain('애니메이션');
  });
  test('알 수 없는 장르는 "전체 장르" 사유가 포함된다', () => {
    expect(makeRecommendReason('공포', '혼밥', '라면')).toContain('전체 장르');
  });
  test('음식 이름이 추천 사유에 포함된다', () => {
    expect(makeRecommendReason('액션', '혼밥', '치킨버거')).toContain('치킨버거');
  });
  test('식사 상황 "혼밥"이 추천 사유에 반영된다', () => {
    expect(makeRecommendReason('액션', '혼밥', '라면')).toContain('혼밥');
  });
  test('식사 상황 "야식"이 추천 사유에 반영된다', () => {
    expect(makeRecommendReason('코미디', '야식', '떡볶이')).toContain('야식');
  });
  test('식사 상황 "친구와 함께"가 추천 사유에 반영된다', () => {
    expect(makeRecommendReason('스릴러', '친구와 함께', '치킨')).toContain('친구');
  });
  test('식사 상황 "연인과 함께"가 추천 사유에 반영된다', () => {
    expect(makeRecommendReason('로맨스', '연인과 함께', '마카롱')).toContain('연인');
  });
  test('식사 상황 "든든한 식사"가 추천 사유에 반영된다', () => {
    expect(makeRecommendReason('액션', '든든한 식사', '비빔밥')).toContain('든든한');
  });
  test('빈 문자열이 아닌 결과를 반환한다', () => {
    expect(makeRecommendReason('전체', '', '김밥').length).toBeGreaterThan(0);
  });
});
